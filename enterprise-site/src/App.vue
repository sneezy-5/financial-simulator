...
<div class="w-full font-body-md text-on-surface antialiased overflow-x-hidden">
  ...
  <header class="fixed top-0 w-full z-50 bg-white/85 backdrop-blur-xl border-b border-slate-200/70 transition-all">
    <div class="h-20 max-w-[1280px] mx-auto px-6 lg:px-8 flex items-center justify-between">
      <div class="flex items-center gap-10">
        <a class="flex items-center gap-3.5 group" :href="appUrl()">
          <div class="relative flex items-center justify-center w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-50 to-amber-50/80 border border-emerald-100 shadow-sm group-hover:scale-105 group-hover:shadow-md transition-all">
            <img alt="Logo Eonda RH" class="h-8 w-auto object-contain drop-shadow-sm" src="/logo.png" />
          </div>
          <div class="flex flex-col">
            <span class="font-headline-md font-extrabold text-[21px] text-brand-emerald-dark tracking-tight leading-none group-hover:text-primary-container transition-colors">
              EONDA <span class="text-amber-500 font-extrabold">RH</span>
            </span>
            <span class="text-[11px] font-medium text-slate-500 tracking-wider uppercase mt-1">Paie &amp; RH Côte d'Ivoire</span>
          </div>
        </a>
      ...
      <div class="flex items-center gap-3">
        <button @click="$emit('login')" class="hidden sm:inline-flex items-center px-4 py-2.5 rounded-xl text-[14px] font-semibold text-slate-700 hover:text-brand-emerald-dark hover:bg-slate-100 transition-colors">
          Se connecter
        </button>
        <a :href="appUrl('hr')" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 via-primary-container to-emerald-700 text-white font-headline-sm font-semibold text-[14px] shadow-[0_4px_14px_rgba(15,91,56,0.25)] hover:shadow-[0_6px_20px_rgba(15,91,56,0.35)] hover:-translate-y-0.5 active:translate-y-0 transition-all">
