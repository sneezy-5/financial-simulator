...
async function main() {
  ...
  const companyInfo = {
    ...
    bulletinCouleur: '#be123c'
  };

  const buf = await payrollService.generateSinglePdf(employee, calc, companyInfo, null, 'personnalise');
  fs.writeFileSync(process.env.OUT_DIR + '/bulletin_personnalise.pdf', buf);
